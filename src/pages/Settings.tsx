...
// Detect if running as Electron desktop app
...
function BackendStatus() {
  ...
  const handleRestart = async () => {
    setRestarting(true);
    try {
      if ((window as any).electronAPI?.restartBackend) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-base-content/50 font-mono">{window.location.origin}</span>
      </div>
